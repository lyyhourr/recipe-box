            ...
            {Links.map((link, index) =>
              link.subCategory ? (
                <button
                  ...
                  key={index}
                  className="transition-all text-start duration-100 ease-in-out"
                >
                  <div className="flex items-center gap-2">
                    {link.name}
                    <svg
                      className={cn(
                        "transition-all text-start duration-100 ease-in-out",
                        { "rotate-180": index === isOpen }
                      )}
                      viewBox="0 0 8 6"
                      height={12}
                      width={14}
                  ...
                  <div
                    className={cn(
                      "max-h-0 overflow-hidden flex flex-col gap-5",
                      { "max-h-fit my-1 py-3 mx-5": isOpen === index }
                    )}
                  >
                    {link.subCategory.map((sub) => (
                      <Link
                        className="hover:underline underline-offset-8 text-2xl decoration-4"
                        key={sub}
                        href={`/${
                          link.name === "Cuisines" ? "cuisine" : "mealtype"
                        }/${sub}`}
                      >
                        {sub}
                      </Link>
                ...
                <Link
                  href={link.path}
                  key={index}
                  className="hover:opacity-75 font-normal transition-all duration-100 ease-in-out"
                >
                  {link.name}
                </Link>
              )
